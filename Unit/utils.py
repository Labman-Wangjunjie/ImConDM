...
def metrics_calculate(values, re_values, labels,masks, mind, ts, th1, th2):
    ...
    i = 0
    result = t.chunk(re_values, len(mind), dim=0)
    mask_ = t.chunk(masks,len(mind),dim = 0)

    print('******')

    for chunk,mask in zip(result,mask_):
        print(chunk.shape)
        chunks =chunk.numpy()
        np.savetxt('./test/tensor{}.csv'.format(i), chunks, delimiter=',')
        score = anomaly_scoring(values * (1 - mask), chunk * (1 - mask))
        scores = scores + (score*mind[i])
        i = i + 1

    preds, _ = evaluate(labels, scores, th1, adj=False)
    preds_,_ = evaluate(labels, scores, th2, adj=True)
...
def evaluate(labels, scores, th, adj=True):
    f1 = 0.0
    preds = None
    # 在max和min之间隔开为1000个数，迭代用数值作为阈值计算f1
